...
def filterSumStats(pvals,ss,):
    ...
    for p in pvals:
        samp = ss[(ss[PVAL] <= p)]
        pvals_ref[p] = [samp.index.tolist(), samp[PVAL].values] #Get just the pvalues and the indices.
    return pvals_ref
...
if __name__ == '__main__':
    ...
    #Now do it 
    scores = dict.fromkeys(pvals, []) #should make a dictionary of pvalue lists.
    patient_ids = list()
    print("reading the snps in....")
    with open(snp_matrix, 'r') as istream:
        ...
        #for line in istream:
            ...
            if counter == 0:
                ...
                line = istream.readline().strip()
                counter += 1
                print ("header line extracted")
                continue
            dat = np.array(line.split(), dtype = np.float64)
            patient_ids.append(dat[0])
            dat = dat[6:] #Just the numberic values at the end.
            for p in pvals:
                vals = dat[snp_indices[p][INDEX]] #Select out just those indices
                score = np.dot(vals, snp_indices[p][B])
