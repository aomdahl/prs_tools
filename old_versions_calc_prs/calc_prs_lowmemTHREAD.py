...
N_CORES = 1
"""
Functions for threading the file parsing
Based on blog by Oxford Protein Informatics Group
https://www.blopig.com/blog/2016/08/processing-large-files-using-python/
Note, just getting the baseline version of this working, can improve by reducing the open calls we make
I need a better way to organize this- too many arguments being passed. Make an object?
"""
def lgpWrapper(snp_mat, lineByte, pvals, var_map,snp_indices, scores, patient_ids, ss_ids):
    with open(snp_mat) as f:
        #Need to put a try catch here, but unsure of what kind of error it might be... 
        f.seek(lineByte)
        line = f.readline().strip().split()
        #Processing now
        #SHOULD BUILD var map on first open of file
        #if counter == 0: #Build the map if we need to
        #    var_map = buildVarMap(line[6:], ss_ids)
        #else:
        patient_ids.append(line[0])
            #This should update the dictionary as it goes, hopefully modifying it along the way >_<
        calcPRSLine(line, pvals, var_map, snp_indices, scores)
        
#Dat is already split!
def calcPRSLine(dat, pvals, var_map, snp_indices, scores):
...
def populateLineDiffs(diff_tab, patient_num):
    diff = diff_tab[-1] - diff_tab[-2]
    for i in range(len(diff_tab)-2, patient_num):
        diff_tab.append(diff_tab[-1] + diff)
    print("Number of lines we have data for", len(diff_tab))
    return diff_tab       

def inferLineDiff(fin, n, ss_ids):
    """Avoid parsing the whole file by getting the consistent line length
        Assumes the same from one line to the next, but does check the 2nd and 3rd lines
    """
    diff_tab = list()
    running_len = 0
    with open(fin) as istream:
        nextLineByte = istream.tell()
        diff_tab.append(nextLineByte)
        counter = 0
        var_map = None 
        for line in istream:
            ...
            if counter ==  0:
                ...
                #Generate the var map
                print("var_map generated!")
                var_map = buildVarMap(line[6:], ss_ids)
            if len(diff_tab) >= 4:
                if diff_tab[-1] - diff_tab[-2] == diff_tab[-2] - diff_tab[-3]:
                    return populateLineDiffs(diff_tab,n), var_map
            counter += 1
            if counter > 5:
                print("We detect something wrong with your plink data file, please takea  closer look.")
                
def lgpParallel(snp_matrix, snp_indices,pvals, scores, ss_ids, patient_count):
    print("Running the analysis in parallel across",N_CORES, "threads")
    patient_ids = list()
    pool = mp.Pool(N_CORES)
    jobs = [] 
    var_map = None
    file_pos = 0
    print('Populating line differences...')
    #Okay, this is a bad coding practice, but we can get the var map out while we are parsing this once and be efficient. So that is what we are doing.    
    line_diffs, var_map = inferLineDiff(snp_matrix + ".raw", patient_count, ss_ids)
    
    """
    with open(snp_matrix + ".raw") as istream:
        nextLineByte = istream.tell()
        print(nextLineByte)
        counter = 0
        for line in istream: 
            #Question, doesnt this go extra slow since it reads in each line twice? Seems painful to me. All we are interested in are the line lengths
            jobs.append(pool.apply_async(lgpWrapper,(snp_matrix,nextLineByte, pvals,var_map, snp_indices, scores, patient_ids, counter, ss_ids)))
            #nextLineByte = istream.tell()
            nextLineByte += len(line)
            print(nextLineByte)
            counter += 1
    ...
    #More efficient, avoid reading in everything
    print("Populating job list...")
    for entry in line_diffs[1:]: #omit the first entry, its just 0, already been processed.
            jobs.append(pool.apply_async(lgpWrapper, (snp_matrix + ".raw", entry, pvals, var_map, snp_indices, scores, patient_ids, ss_ids)))
    print("Calling asynchronous jobs")
    for job in jobs:
        print(job)
        job.get()
        cpname = mp.current_process()
        print(cpname)
    print('All the jobs gotten')
    pool.close()
...
def getPatientCount(snp_file, vers):
    try:
        if vers == 1:
            return sum(1 for line in open(snp_file + ".fam"))
        return sum(1 for line in open(snp_file + ".psam")) - 1
    except:
...
def calculatePRS(patient_count, pvals, snp_matrix, snp_indices, snp_list):
    #Seraj shoutout
    start = time.time()
    scores = { p : [] for p in pvals}
    if N_CORES > 1:
       scores, patient_ids, debug_tab = lgpParallel(snp_matrix, snp_indices,pvals, scores, snp_list, patient_count)
    else: 
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--ncores", type = int, default = 1, help = "Specify the number of cores to use")
    ...
    patient_count = getPatientCount(args.plink_snps, args.plink_version)
    ...
    scores, patient_ids, debug_dat = calculatePRS(patient_count, pvals, snp_matrix, snp_indices,stats_complete[REFID])
