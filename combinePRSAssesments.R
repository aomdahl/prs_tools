...
outpath <- args$output
low_corr <- read_tsv(low) %>% mutate(source= "low")  %>% mutate(pval_names = as.factor(pval_names))
high_corr <- read_tsv(high) %>% mutate(source = "phigh")%>% mutate(pval_names = as.factor(pval_names))
if(args$mixed_results == "")
{
    fin <- rbind(low_corr, high_corr) #%>% mutate(bonf_p = p.adjust(pval_beta, method ="bonferroni"))
    ggplot(data = fin, aes((pval_names), r2, fill = source)) + geom_bar(stat = "identity", position = "dodge") + xlab("GWAS Pvalue thresholds") + ylab(expression(paste("R"^"2"))) + scale_fill_discrete(name = "Sample sets", labels = c(args$label_1, args$label_2))
}else{
if (args$multi_mixed != "")
{
...
mixed <- read_tsv(args$mixed_results) %>% mutate(source= "std")  %>% mutate(pval_names = as.factor(pval_names))
fin <- rbind(low_corr, high_corr, mixed)
ggplot(data = fin, aes((pval_names), r2, fill = source)) + geom_bar(stat = "identity", position = "dodge") + xlab("GWAS Pvalue thresholds") + ylab(expression(paste("R"^"2"))) + scale_fill_discrete(name = "Sample sets", labels = c(args$label_1, args$label_2, args$label_mixed))
}
}
